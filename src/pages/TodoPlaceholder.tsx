...
export default function TodoPlaceholder() {
  ...
  const title = useMemo(
    () => (params.slug ? formatSlug(params.slug) : t("pages.todoPlaceholder.pendingContent")),
    [params.slug, t],
  );

  return (
    <section className="mx-auto flex min-h-[60vh] w-full max-w-4xl flex-col items-start justify-center px-4 py-16 md:px-6">
      <p className="text-sm font-semibold uppercase tracking-wide text-amber-700">{t("pages.todoPlaceholder.todoLabel")}</p>
      <h1 className="mt-2 text-3xl font-semibold text-slate-900">{title}</h1>
      <p className="mt-3 text-slate-600">{t("pages.todoPlaceholder.routeMessage")}</p>
      <Link
        to="/"
